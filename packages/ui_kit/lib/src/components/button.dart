...
import 'package:ui_kit/src/components/loader.dart';
import 'package:ui_kit/src/theme/colors.dart';

enum VmButtonStyle { primary, secondary, outline, ghost }

enum VmButtonRole { normal, accent, destructive }
...
class VmButton extends StatelessWidget {
  ...
  _resolveColors(ThemeData theme) {
    ...
    final tokens = VmThemeColors.byBrightness(theme.brightness);
    ...
    final roleForeground = switch (role) {
      VmButtonRole.normal => tokens.textPrimary,
      VmButtonRole.accent => tokens.primary,
      VmButtonRole.destructive => tokens.danger,
    };
    final secondaryBackground = switch (role) {
      VmButtonRole.normal => tokens.surfaceHigh,
      VmButtonRole.accent => tokens.primaryMuted,
      VmButtonRole.destructive => tokens.dangerMuted,
    };
    ...
    final primaryForeground = switch (role) {
      ...
      VmButtonRole.accent || VmButtonRole.destructive => tokens.onPrimary,
    };
    ...
    return switch (style) {
      ...
      VmButtonStyle.secondary => (
        ...
        disabledBg: dim(secondaryBackground),
        fg: roleForeground,
        disabledFg: dim(roleForeground),
        border: null,
        overlay: overlay(roleForeground),
        shadow: tokens.transparent,
        elevation: 0.0,
      ),
      VmButtonStyle.outline => (
        bg: tokens.transparent,
        disabledBg: tokens.transparent,
        fg: roleForeground,
        disabledFg: dim(roleForeground),
        border: role == VmButtonRole.normal ? tokens.borderStrong : roleForeground,
        overlay: overlay(roleForeground),
        shadow: tokens.transparent,
        elevation: 0.0,
      ),
  ...
  Widget _buildLabelButton(
    ...
    final buttonStyle = FilledButton.styleFrom(
      ...
      shape: shape,
      alignment: alignment,
      textStyle: theme.textTheme.labelLarge?.copyWith(fontWeight: emphasized ? FontWeight.w600 : FontWeight.w500),
    );

    return ConstrainedBox(
      constraints: BoxConstraints(minHeight: size.height, minWidth: effectiveWidth.value ?? 0.0),
      child: FilledButton(
        style: buttonStyle,
